...
import pyvistaqt
from alfvis_core.constants import default_atom_colors, random_colors
...
class DatasetWidget(QWidget):
    ...
    def __init__(self, xyz_file, plotter: pyvistaqt.QtInteractor):
        ...
        # errors_for_properties is a list of dictionaries.
        # Each dictionary contains property errors for each atom. Could be None if data is not in xyz.
        self.properties = self.trajectory.properties
        self.cmap_properties = (
            self.properties.keys() if self.properties is not None else None
        )
        ...
        # give this widget access to the plotter
        # so the plotter can be modified from each instance separately
    ...
    def current_selected_property(self) -> Union[str, None]:
        """returns the name of the current selected property or `None`
        if no properties were read in from the xyz file."""
    ...
    def default_atom_colors(self) -> dict:
        """Returns a dictionary of default colors for the atoms in the system.
        (e.g. carbon is grey, nitrogen is blue)
    ...
    def actors(self) -> dict:
        """Returns a dictionary of actor names as keys and
        VTK objects as values (these are the things that are plotted)"""
    ...
    def all_noncentral_data(self, central_atom_alf, central_atom_features) -> dict:
        ...
        return self.calculate_non_central_dictionary(
            central_atom_alf, central_atom_features
        )

    def calculate_non_central_dictionary(
        self, central_atom_alf: ALF, central_atom_features: np.ndarray
    ) -> dict:
        """Returns a dictionary of key: atom name, val: np array.
        The np array contains the xyz coordinates of every non-central atom that needs to be plotted.
        The values of the inner dictionary is a 2d-array of the positions of the non-central atoms
        for every timestep (shape n_timesteps x 3)
        ...
        :return: a dictonary of dictionaries containing the
            corresponding non-central data to plot for every central atom
        :rtype: dict
        """
        ...
        xyz_centered_on_central_atom = alf_features_to_coordinates(
            central_atom_features
        )
        previous_atom_ordering = list(range(self.n_atoms))
        current_atom_ordering = list(central_atom_alf) + [
            i for i in range(self.n_atoms) if i not in central_atom_alf
        ]
        ...
        xyz_centered_on_central_atom[
            :, previous_atom_ordering, :
        ] = xyz_centered_on_central_atom[:, reverse_alf_ordering, :]
        ...
        # use a dictionary to store atom names and their coordinates
        # key:atom_name, val: np array of shape n_timesteps x 3
        xyz_dict = {
            atom_name: atom_coords
            for atom_name, atom_coords in zip(
                self.atom_names, xyz_centered_on_central_atom
            )
        }
    ...
    def get_alf_and_features_for_central_atom(self) -> tuple:
        current_central_atom_idx = (
            int(self.current_central_atom_name.strip(string.ascii_letters)) - 1
        )
        currrent_x_axis_idx = (
            int(self.current_x_axis_atom_name.strip(string.ascii_letters)) - 1
        )

        # if molecule has 2 atoms, then there is no xy plane atom
        currrent_xy_plane_idx = None
        if self.current_xy_plane_atom_name:
            currrent_xy_plane_idx = (
                int(self.current_xy_plane_atom_name.strip(string.ascii_letters)) - 1
            )

        alf = ALF(current_central_atom_idx, currrent_x_axis_idx, currrent_xy_plane_idx)
        central_atom_features = self.trajectory[
            self.current_central_atom_name
        ].features(calculate_alf_features, alf)
    ...
    def update_atom_name_combo_boxes(self):
        ...
        self.x_axis_atom_combo_box.addItems(
            [name for name in self.atom_names if name != self.current_central_atom_name]
        )
        self.xy_plane_atom_combo_box.addItems(
            [
                name
                for name in self.atom_names
                if name != self.current_central_atom_name
                and name != self.current_x_axis_atom_name
            ]
        )
    ...
    def use_random_colors(self):
        ...
        for atom_name, button in self.color_buttons_dict.items():
            button.setStyleSheet(
                f"background-color : {self.current_atom_colors[atom_name]}; color: {self.current_atom_colors[atom_name]};"  # noqa: E501
            )
    ...
    def use_default_colors(self):
        ...
        for atom_name, button in self.color_buttons_dict.items():
            button.setStyleSheet(
                f"background-color : {self.default_atom_colors[atom_name]}; color: {self.default_atom_colors[atom_name]};"  # noqa: E501
            )
    ...
    def use_cmap(self):
        ...
        # enable the plot all points radiobutton, because we
        # are using `clicked`, this will not call `plot_all_points` automatically
        ...
        # possibly a way to apply a colormap directly to current
        # vtk objects without making new ones, but that will be a lot of work and little reward
    ...
    def update_central_atom_data(self):
        """Used to update the central ALF atom and the noncentral
        data associated with it, depending on selected atom in combo box"""
    ...
    def enable_plot_individual_points(self):
        ...
        # `checked` method is used to connect to slot, but `checked` does not call
        # the connected method automatically (since it requires user input to call it)
    ...
    def update_individual_point_slider_status_and_box(self):
        """
        Updates the status of the individual point slider depending on the
        checked state of the individual point checkbox. If that checkbox is enabled,
        then only 1 point will be plotted at a time.
        The slider can then be slider to the point of interest in the trajectory.
        """
        ...
        for atom in all_noncentral_data.keys():
            self._current_noncentral_data[atom] = all_noncentral_data[atom][
                current_point
            ]
    ...
    def update_individual_point_slider_value_with_box(self):
        ...
        The slider is updates, so the data is also automatically updated.
        No need to update data to be plotted."""
    ...
    def update_colors_buttons_box(self):
        """updates atoms that are in the color box, depending on
        which central atom is chosen and also which checkboxes are ticked in the checkbox widget."""
    ...
    def show_or_hide_atom(self):
        """Hide the atom from the plotter, as well as hide the atom color box
        that can be used to change atom color. This gets called by every checkbox,
        which has a different atom name, so then if the checkbox is checked,
        the data corresponding to the checkbox name is shown, otherwise the data is hidden."""
    ...
    def hide_all_atoms(self):
        """Hides all atoms by recursively unchecking all checkboxes
        (when a checkbox is unchecked the `self.show_or_hide_atom` method is called.)"""
        for checkbox in self.checkboxes:
            checkbox.setCheckState(QtCore.Qt.Unchecked)

    def show_all_atoms(self):
        """Shows all atoms by recursively checking all checkboxes
        (when a checkbox is checked the `self.show_or_hide_atom` method is called.)"""
        for checkbox in self.checkboxes:
            checkbox.setCheckState(QtCore.Qt.Checked)
