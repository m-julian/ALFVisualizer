...
headings = get_headings(trajectory.nfeatures)
features = trajectory.features # features[point][atom][feature]
features = np.swapaxes(features, 0, 1) # features[atom][point][feature]


dataframes = {}
for atom, data in zip(trajectory.atom_names, features):
    df = pd.DataFrame(data)
    df.index = range(1,len(features[1])+1)
    # edit df here
    dataframes[atom] = df


with pd.ExcelWriter("calculated_features.xlsx") as workbook:
    for atom, df in dataframes.items():
        df.columns = headings
        df.to_excel(workbook, sheet_name=atom)